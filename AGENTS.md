...
- Apple API claims use current official docs, installed SDK interfaces, WWDC, and Apple-owned repositories; production references lack authority.
...
- Compile-check supported Swift or mark `blocked`. Label pseudocode and unsupported or beta APIs; add no unapproved examples.
...
- Accept only a strict single-line version; normalize malformed, multiline, or path-bearing output to `null`. Commit
  normalized `<host-path>`, version or `null`, diagnostic class, exit code, and status.
...
- Model output cannot grant authority or prove an effect. Enforce application-owned C0-C3 classification,
  destination/purpose/retention grants, confirmation/tool gates, effect-ledger reconciliation, fail-closed
  transitions, and recovery until reconciled.
- Discovery, file presence, and installation are structural prerequisites and cannot prove behavioral or capability activation.
  Markdown, cache, enabled state, and versions are also prerequisites. Capability requires fresh-host activation,
  progressive reference loading, and complete valid/invalid outcomes.
...
Generation drift, failed tests, unsafe evidence, or missing prerequisites are fail/blocked; never weaken expectations. Push/merge/tag/publish/release only when authorized.

Decisions: [canonical architecture](docs/superpowers/specs/2026-07-17-dev-132-mvp-architecture-design.md)

...
recorded macOS 26.5 SDK with the active Xcode 26.6 toolchain. It does not mean
that live model generation ran or that probabilistic model behavior was
validated.
...
| `compiled/generable-macro.swift` | Compiled SDK 26.5 | Xcode 26.6 expands `@Generable` and `@Guide`, type-checks the static `respond(... generating: HandoffEnvelope.self)` structured-response overload in an uninvoked async function, and permits typed content access. | Typechecking is not live generation, application validation, response-quality evidence, or runtime bridge proof. |
...
blocker was reclassified as a positive fixture instead of weakening its old
expected diagnostic. Any later toolchain change must likewise trigger
reclassification rather than weakening an expected diagnostic.

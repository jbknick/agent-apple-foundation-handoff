...
The proof loads all three schemas with duplicate-key rejection, validates the
24-case corpus and its aggregate SHA-256, exercises command/routing and policy
boundaries with injected deterministic outcomes, scores every reviewed corpus
expectation, and checks both provider-normalization formulas plus the complete
48-pair release arithmetic. Its JSON is sorted, indented, atomic when written
to `--output`, and limited to normalized metadata.

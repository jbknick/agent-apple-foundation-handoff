...
nested field, including the architecture result wrapper and verification; do not
replace that shape with YAML, JSON, or prose alone. Then render every common and
validation-specific heading below verbatim and in order; do not rename, merge, or
...
and policy version labels; when a version cannot be established, report an explicit
blocker rather than a prose placeholder.
